...
Class Master extends DBConnection {
	...
	function save_records(){
		...
		$check = $this->conn->query("SELECT * FROM `epr` where `id` = '{$id}' and delete_flag = 0 ".(!empty($id) ? " and idh != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['code'] = "Product Code already exists. Code must be unique";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
			$sql = "INSERT INTO `epr` set {$data} ";
		}else{
			$sql = "UPDATE `epr` set {$data} where id = '{$id}' ";
		}
